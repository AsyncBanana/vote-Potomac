...
import MilestoneTemplate from "../../../email/milestone.mjml";
import { Users } from "../../../schemas/user";
const VoteMilestones = [3, 5, 10, 15, 25, 50, 75, 100];
export const POST: APIRoute = async (ctx) => {
	...
	const [[res], [authorData]] = await ctx.locals.db.batch([
		...
		ctx.locals.db
			.select({
				...
				milestoneNotifications: Users.milestoneNotifications,
				name: Users.name,
				email: Users.email,
			})
			.from(Users)
	...
	if (res) {
		if (authorData.voteNotifications) {
		...
		if (
			vote === "up" &&
			VoteMilestones.includes(res.voteCount || 0) &&
			authorData.milestoneNotifications
		) {
			ctx.locals.runtime.ctx.waitUntil(
				sendEmail(
					{
						to: [authorData.email],
						subject: `Your ${type} has reached ${res.voteCount || 0} votes!`,
						html: MilestoneTemplate.replaceAll(
							"{{ message }}",
							`Your ${type} "${res.title}" has been voted for ${res.voteCount || 0} times! Congrats!`,
						)
							.replaceAll("{{ votes }}", (res.voteCount || 0).toString())
							.replaceAll("{{ type }}", type),
					},
